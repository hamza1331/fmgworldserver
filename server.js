...
//Add Featured video
app.post('/api/addFeaturedVideo',(req,res)=>{ //tested
	uploadMult(req,res,function(err) {
		if(err) {
      console.log(err)
			return res.json(handleErr(err))
      ...
      let fileData = req.files
      let thumbnail = fileData[0].mimetype.includes('image')===true?fileData[0]:fileData[1]
      let videoFile = fileData[0].mimetype.includes('image')===true?fileData[1]:fileData[0]
    data.fileServerName = videoFile.filename
    data.thumbnail = thumbnail.filename
    data.isVideo=true
